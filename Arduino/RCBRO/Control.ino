...
float speedPIControl(float DT, int16_t input, int16_t setPoint, float Kp, float Ki, bool integrate = true)
{
    ...
    if (integrate)
    {
        float limitedError = constrain((float)error, -ITERM_MAX_ERROR, ITERM_MAX_ERROR);
        PID_errorSum += limitedError * DT;
...
float positionPDControl(long actualPos, long setPointPos, float Kpp, float Kdp, int16_t speedM)
{
    ...
    // Damping should usually oppose movement.
    // If your robot reacts backwards, change '-' back to '+'.
...
ISR(TIMER1_COMPA_vect)
{
    ...
    // Minimum STEP high pulse width for stepper driver
    asm volatile(
        "nop\nnop\nnop\nnop\n"
        "nop\nnop\nnop\nnop\n"
    );
...
void setMotorSpeed(uint8_t motorNum, int16_t tspeed)
{
...
#if MICROSTEPPING == 16
    speed = (int32_t)speed_M * 50L;
#else
    speed = (int32_t)speed_M * 25L;
#endif
    ...
    {
        timer_period = 2000000L / speed;

        if (timer_period > 65535L)
        {
            timer_period = ZERO_SPEED;
            dir_M = 0;
        }
    ...
    uint16_t ocr = (uint16_t)timer_period;
